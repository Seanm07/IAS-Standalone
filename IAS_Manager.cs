...
public class IAS_Manager : MonoBehaviour {
    ...
    private IEnumerator AsyncLoadAnimatedTextures(int adTextureId, string filePath, string fileName) {
	    ...
	    byte[] animatedTextureBytes = File.ReadAllBytes(filePath + fileName);
	    ...
	    if (needToRebuildAnimationFrames) {
		    ...
		    for (int i = 0; i < animationFrames; i++) {
			    ...
			    if (animationFrameTex.LoadImage(curFrameBytes)) { // Expensive operation
				    if (compressLoadedTextures) {
					    // Make sure the cached texture is already POT sized
					    if (Mathf.IsPowerOfTwo(animationFrameTex.width) && Mathf.IsPowerOfTwo(animationFrameTex.height)) {
						    animationFrameTex.Compress(true);
	...
	public void StoreInitializeFinished() {
		...
		switch (store) {
			...
			case AppStore.MacAppStore:
				...
				if (isGameShark) {
					activeJsonURL = "https://ias.gamepicklestudios.com/ad/3.json";
					activeSlotId = 3;
				} else {
					activeJsonURL = "https://ads2.gumdropgames.com/ad/9.json";
					activeSlotId = 9;
				}
			...
			case AppStore.AmazonAppStore:
				activeJsonURL = "https://ias.gamepicklestudios.com/ad/2.json";
				activeSlotId = 2;
				break;

			case AppStore.UDP:
				switch (CrossPlatformManager.GetActiveUDPStore()) {
					case UDPStore.SAMSUNGGALAXYSTORE:
						activeJsonURL = "https://ias.gamepicklestudios.com/ad/11.json";
						activeSlotId = 11;
						break;
					case UDPStore.ONESTORE:
						activeJsonURL = "https://ias.gamepicklestudios.com/ad/12.json";
						activeSlotId = 12;
						break;
					case UDPStore.XIAOMI:
					case UDPStore.XIAOMISTORE:
					case UDPStore.XIAOMISTORECN:
						activeJsonURL = "https://ias.gamepicklestudios.com/ad/13.json";
						activeSlotId = 13;
						break;
					case UDPStore.HUAWEI:
						activeJsonURL = "https://ias.gamepicklestudios.com/ad/14.json";
						activeSlotId = 14;
						break;
					case UDPStore.QOOAPP:
						activeJsonURL = "https://ias.gamepicklestudios.com/ad/15.json";
						activeSlotId = 15;
						break;
					case UDPStore.JIO:
					case UDPStore.JIOGAMESSTORE:
						activeJsonURL = "https://ias.gamepicklestudios.com/ad/16.json";
						activeSlotId = 16;
						break;
					case UDPStore.UPTODOWN:
						activeJsonURL = "https://ias.gamepicklestudios.com/ad/17.json";
						activeSlotId = 17;
						break;
					case UDPStore.LEGIONREALM:
						activeJsonURL = "https://ias.gamepicklestudios.com/ad/18.json";
						activeSlotId = 18;
						break;
					case UDPStore.APPTUTTI:
						activeJsonURL = "https://ias.gamepicklestudios.com/ad/19.json";
						activeSlotId = 19;
						break;
					case UDPStore.SHAREIT:
						activeJsonURL = "https://ias.gamepicklestudios.com/ad/20.json";
						activeSlotId = 20;
						break;
			...
			case AppStore.GooglePlay:
				...
				if (isGamePickle) {
					...
					activeJsonURL = "https://ias.gamepicklestudios.com/ad/" + (platform == Platform.Standard ? "1" : "6") + ".json";
					activeSlotId = (platform == Platform.Standard ? 1 : 6);
				} else {
					// (Gumdrop Games) 4 = Google Play Standard / 8 = Google Play Android TV
					activeJsonURL = "https://ads2.gumdropgames.com/ad/" + (platform == Platform.Standard ? "4" : "8") + ".json";
					activeSlotId = (platform == Platform.Standard ? 4 : 8);
				}
	...
	public void Update() {
		...
		if (Input.GetKeyDown(KeyCode.R)) {
			RefreshBanners(IASAdSize.Square, true);
			RefreshBanners(IASAdSize.Tall, true);
		}
#endif
	}
	...
	private IEnumerator DownloadAdTexture(IASAdSize adSize) {
		...
		for (int i = 0; i < maxAdOffset + 1 && (i < advertData.slotInts[(int)adSize - 1].advert.Count); i++) {
			...
			char slotChar = GetSlotChar(adSize, i);
			AdData curAdData = GetAdDataByChar(adSize, slotChar);

			if (advancedLogging)
				Debug.Log("Request to load " + (int)adSize + "" + slotChar);
		...
		if (curSlotData != null) {
			...
			if (customData == null) {
				...
				for (int i = 0; i <= totalAds; i++) {
					int slotCharIdCheck = Mathf.Abs(lastSlotId + i) % curSlotData.advert.Count;
					char slotCharCheck = (char)(slotCharIdCheck + slotIdDecimalOffset);
					
					AdData curAd = GetAdDataByChar(adSize, slotCharCheck);

					if (!curAd.isSelf && curAd.isActive && !curAd.isInstalled && !displayableAdPackageNames.ContainsKey(curAd.slotChar) && !displayableAdPackageNames.ContainsValue(curAd.packageName)) {
				...
				if (includeInstalledGames) {
					...
					for (int i = 0; i <= totalAds; i++) {
						...
						if (!curAd.isSelf && curAd.isActive && !displayableAdPackageNames.ContainsKey(curAd.slotChar) && !displayableAdPackageNames.ContainsValue(curAd.packageName)) {
							displayableAdPackageNames.Add(curAd.slotChar, curAd.packageName);
							// E, A, B, C, D
						}
					}
			...
			int wantedSlotCharId = (lastSlotId + finalOffset + offset) % curSlotData.advert.Count; // If we use modulo here then empty ad slots would have ads but it would mean duplicate ads could appear on the backscreen together
